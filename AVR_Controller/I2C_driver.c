...
uint8_t i2c_0_GetData(uint8_t address, uint8_t *pData, uint8_t len)
{
	...
	if ((len != 0) && (pData != 0))
	{
		while (len--)
		{
			...
			{
				*pData = TWI0.MDATA; // Save the acquired data in the memory location pointed

				if (len == 0) // Evaluate if it's now time to end the comunication
				{
				...
				retVal++; // Increment the number of read Bytes
				pData++;  // Incrment the pointer to access the next memory location
...
uint8_t i2c_0_WriteData(uint8_t address, uint8_t *pData, uint8_t len)
{
	uint8_t retVal = (uint8_t)-1;

	/* start transmitting the client address */
	TWI0.MADDR = address & ~0x01; // Complement 0x01 -> 11111110 & Address, this will keep the 1's and leave a 0 in the 0x01 bit
	if (i2c_0_WaitW() != I2C_ACKED)
		return retVal;
